...
% Generate random scores to which the distribution will be fitted:
n1=100;  % arbitrary sample size
Datasets{1} = normrnd(55,2,n1,1);
% Note that the constraint is not truly satisfied because the true sigma is less than 5.5.

% Set up the fitting problem:
Dists{1} = Normal(45,4);       % The distributions to be fit.  These parameter values are not actually used.
...
% Generate random scores to which each distribution will be fitted:
...
disp('This example requires also requires construction of complicated Datasets,');
disp('because the mAFCProbitLnLikelihood error function requires multiple inputs.');
...
pGuess = 1/m;
s = 50;  % Use a constant of s=50 (also used in DemoFitConstraintFn4)

% Construct some artificial data to which the distributions will be fit:
...
% Note that the constraint is truly satisfied because CDF1(s)+CDF2(s)= 1 at s=50.
...
% Make the datasets that will be passed to mAFCProbitLnLikelihood.
% Each dataset specifies mAFCProbitLnLikelihood's required parameters, in order.
% Here as a reminder is the function definition:
