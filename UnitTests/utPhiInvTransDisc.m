classdef utPhiInvTransDisc < utDiscrete;
    ...
    methods
        ...
        function testCase=utPhiInvTransDisc(varargin)  % Constructor
    ...
    methods (TestClassSetup, ParameterCombination='sequential')
        ...
        function ClassSetup(testCase,parmCase)
            ...
            switch parmCase
                case 1
                    testCase.Dist = PhiInvTrans(LinearTrans(Binomial(53,.44),1/154,.25));
                    testCase.EstParmCodes = 'frff';
                case 2
                    testCase.Dist = PhiInvTrans(LinearTrans(BinomialMixed([(1:10)/15]),0.07,0.05));
%                     testCase.EstParmCodes = 'ffffffffffr';  % P's cannot be varied but they are counted as parameters
                    testCase.SkipEstAll = true;
                case 3
                    testCase.Dist = PhiInvTrans(MultTrans(Geometric(0.1),1e-8));
                    testCase.EstParmCodes = 'rf';
                case 4
                    p = Poisson(23.5);
                    testCase.Dist = PhiInvTrans(LinearTrans(p,1/(1.5*p.UpperBound),.01));
                    testCase.EstParmCodes = 'frf';
                case 5
                    testCase.Dist = PhiInvTrans(LinearTrans(UniformInt(0,100),.005,.01));
