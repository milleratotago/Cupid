classdef Ratio < dContinuous  % dEither %  Only continuous & POSITIVE so far.  Handle Ratios of discrete RVs via ListRV mechanism?
    % Ratio(BasisRV1,BasisRV2) creates a random variable that is
    %  the Ratio of the two independent POSITIVE basis random variables,
    %  BasisRV1/BasisRV2
    ...
    methods
        ...
        function obj=Ratio(varargin)
            obj=obj@dContinuous('Ratio');
            switch nargin
                case 0
                case 2
                    ...
                    if (obj.BasisRV1.DistType=='c') && (obj.BasisRV2.DistType=='c')
                        ...
                        assert(false,'Ratio can only handle continuous Basis distributions (so far)');
                    end
                ...
                otherwise
                    ME = MException('Ratio:Constructor', ...
                        'Ratio constructor needs 0 or 2 arguments.');
                    throw(ME);
            end
        end
        
        function BuildMyName(obj)
            obj.StringName = ['Ratio(' obj.BasisRV1.StringName ',' obj.BasisRV2.StringName ')'];
        end
        ...
        function thiscdf=CDF(obj,X)
            ...
            for iel=1:numel(X)
                if InBounds(iel)
                    ...
                    thiscdf(iel)=integral(fun,obj.BasisRV2.LowerBound,obj.BasisRV2.UpperBound);
                end
            end
        end
